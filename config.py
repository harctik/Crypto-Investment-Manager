...
if _IS_VERCEL:
    ...
    _SRC_DB = Path(__file__).parent / "data" / "crypto_data.db"
    _DST_DB = DATA_DIR / "crypto_data.db"
    if _SRC_DB.exists() and not _DST_DB.exists():
...
MIX = {
    "iterations":     _int("MIX_ITERATIONS", 1000),
    "min_weight":     0.05,
    "risk_free_rate": _float("RISK_FREE_RATE", 0.02),
    "history_limit":  60,
    "frontier_points": 50,          # points on efficient frontier
    "annualise_factor": 365,        # crypto trades 365 days/year
}
...
SPREAD = {
    ...
    "monte_carlo_sims":     _int("MONTE_CARLO_SIMS", 10000),
    "stress_scenarios": {
        "bull_market":   30.0,
        "bear_market":  -40.0,
        "flash_crash":  -20.0,
...
DATABASE["retention_days"] = _int("DB_RETENTION_DAYS", 90)